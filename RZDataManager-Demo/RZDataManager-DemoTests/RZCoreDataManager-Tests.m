...
- (void)test200ImportObject
{
    ...
    [self.dataManager importData:mockData forClassNamed:@"DMEntry" options:@{RZDataManagerReturnObjectsFromImport : @(YES)} completion:^(id result, NSError *error)
    {
...
- (void)test200bImportObject_importOverride
{
    ...
    __block BOOL finished = NO;
    [self.dataManager importData:mockData forClassNamed:@"DMCustomEntry" options:@{RZDataManagerReturnObjectsFromImport : @(YES)} completion:^(id result, NSError *error)
     {
         STAssertNotNil(result, @"Result should not be nil");
         STAssertNil(error, @"Error during import: %@", error);
...
- (void)test201bImportMultipleObjects_deleteStale
{
    ...
    NSDictionary *options = @{RZDataManagerDeleteStaleItemsPredicate : [NSPredicate predicateWithFormat:@"createdDate > %@", [NSDate dateWithTimeIntervalSinceReferenceDate:0]],
            RZDataManagerReturnObjectsFromImport : @(YES)};
    __block BOOL finished = NO;
...
- (void)test202ImportObjectWithOverriddenMapping
{
    ...
    __block BOOL finished = NO;
    [self.dataManager importData:mockData forClassNamed:@"DMEntry" keyMappings:@{ @"mahNameIs" : @"name" } options:@{RZDataManagerReturnObjectsFromImport : @(YES)}  completion:^(id result, NSError *error)
     {
         STAssertNotNil(result, @"Result should not be nil");
         STAssertNil(error, @"Error during import: %@", error);
...
- (void)test207AbandonChanges
{
    ...
    finished = NO;
    [self.dataManager importData:mockData forClassNamed:@"DMEntry" options:@{RZDataManagerReturnObjectsFromImport : @(YES), RZDataManagerSaveAfterImport : @(NO)} completion:^(id result, NSError *error)
     {
         STAssertNotNil(result, @"Result should not be nil");
         STAssertNil(error, @"Error during import: %@", error);
